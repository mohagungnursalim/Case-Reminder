                        ...
                        <div class="col-lg-4">
                            <div class="input-group input-group-outline my-3">
                                ...
                                <input type="text" name="search" value="{{request('search')}}"
                                    placeholder="Cari Agenda.." class="form-control">
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm bg-warning" data-bs-toggle="modal"
                                data-bs-target="#editModal{{ $reminder->id }}">
            ...
            <div class="modal-body">
                <form action="{{ route('agenda.update', $reminder->id) }}" method="POST">
                    ...
                    <div class="input-group input-group-outline @error('lokasi') is-invalid @enderror">
                        <select required class="form-control" name="lokasi" id="lokasi">
                            ...
                            <option value="Kejari Poso"
                                {{ old('lokasi', $reminder->lokasi) == 'Kejari Poso' ? 'selected' : '' }}>Kejari Poso
                            </option>
                            ...
                            <option value="Kejari Buol"
                                {{ old('lokasi', $reminder->lokasi) == 'Kejari Buol' ? 'selected' : '' }}>Kejari Buol
                            </option>
                            <option value="Kejari Morowali"
                                {{ old('lokasi', $reminder->lokasi) == 'Kejari Morowali' ? 'selected' : '' }}>Kejari
                                Morowali</option>
                        </select>
                    </div>
                    @error('lokasi')
                    ...
                    <div class="input-group input-group-outline @error('nama_kasus') is-invalid @enderror mb-1">
                        <select required id="nama_kasus{{ $reminder->id }}" name="nama_kasus" style="width: 100%;"
                            class="form-control">
                            <option value="" disabled>---Pilih Kasus---</option>
                            @foreach($kasuss as $kasus)
                            <option value="{{ $kasus->nama }}"
                                {{ $reminder->nama_kasus == $kasus->nama ? 'selected' : '' }}>{{ $kasus->nama }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-outline @error('nama_atasan') is-invalid @enderror mb-1">
                        <select required id="nama_atasan{{ $reminder->id }}" name="nama_atasan[]" style="width: 100%;"
                            multiple class="form-control">
                            @foreach($atasans as $atasan)
                            <option value="{{ $atasan->nama }}"
                                {{ in_array($atasan->nama, $selectedNamaAtasan) ? 'selected' : '' }}>{{ $atasan->nama }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-outline @error('nomor_atasan') is-invalid @enderror mb-1">
                        <select required id="nomor_atasan{{ $reminder->id }}" name="nomor_atasan[]" style="width: 100%;"
                            multiple class="form-control">
                            @foreach($atasans as $atasan)
                            <option value="{{ $atasan->nomor_wa }}"
                                {{ in_array($atasan->nomor_wa, $selectedNomorAtasan) ? 'selected' : '' }}>
                                {{ $atasan->nama }} ({{ $atasan->nomor_wa }})</option>
                            @endforeach
                        </select>
                    </div>
                    @error('nomor_atasan')
                    ...
                    <div class="input-group input-group-outline @error('nama_jaksa') is-invalid @enderror mb-1">
                        <select required id="nama_jaksa{{ $reminder->id }}" name="nama_jaksa[]" style="width: 100%;"
                            multiple class="form-control">
                            @foreach($jaksas as $jaksa)
                            <option value="{{ $jaksa->nama }}"
                                {{ in_array($jaksa->nama, $selectedNamaJaksa) ? 'selected' : '' }}>{{ $jaksa->nama }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-outline @error('nomor_jaksa') is-invalid @enderror mb-1">
                        <select required id="nomor_jaksa{{ $reminder->id }}" name="nomor_jaksa[]" style="width: 100%;"
                            multiple class="form-control">
                            @foreach($jaksas as $jaksa)
                            <option value="{{ $jaksa->nomor_wa }}"
                                {{ in_array($jaksa->nomor_wa, $selectedNomorJaksa) ? 'selected' : '' }}>
                                {{ $jaksa->nama }} ({{ $jaksa->nomor_wa }})</option>
                            @endforeach
                        </select>
                    </div>
                    @error('nomor_jaksa')
                    ...
                    <div class="input-group input-group-outline @error('nama_saksi') is-invalid @enderror mb-1">
                        <select required id="nama_saksi{{ $reminder->id }}" name="nama_saksi[]" style="width: 100%;"
                            multiple class="form-control">
                            @foreach($saksis as $saksi)
                            <option value="{{ $saksi->nama }}"
                                {{ in_array($saksi->nama, $selectedNamaSaksi) ? 'selected' : '' }}>{{ $saksi->nama }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-outline @error('pesan') is-invalid @enderror">
                        <textarea required class="form-control" rows="5" name="pesan" id="pesan"
                            required>{{ old('pesan', $reminder->pesan) }}</textarea>
                    </div>
                    @error('pesan')
                    <p class="text-bold text-xs text-danger">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="input-group input-group-outline @error('tanggal_waktu') is-invalid @enderror">
                        <input required class="form-control" type="datetime-local" name="tanggal_waktu"
                            id="tanggal_waktu"
                            value="{{ old('tanggal_waktu', date('Y-m-d\TH:i', strtotime($reminder->tanggal_waktu))) }}"
                            required>
                    </div>
                    <p class="text-xs text-bold text-secondary mt-2">*PM Mulai dari 00.00 hingga 11.59 | AM Mulai dari
                        12.00 hingga 23.59</p>
                    @error('tanggal_waktu')
                    <p class="text-bold text-xs text-danger">{{ $message }}</p>
                    @enderror
...
<div class="modal fade" id="deleteModal{{ $reminder->id }}" tabindex="-1"
    aria-labelledby="deleteModalLabel{{ $reminder->id }}" aria-hidden="true">
